...
var Lances_arrematado = require('../lances_arrematados/lances_arrematados.model');
...
exports.disponiveis = function (req, res) {
    ...
    var user = req.user,
        page = req.body.page || 0,
        query = {},
        fields = { image: 1, referencia: 1, description: 1, categoria: 1, subCategoria: 1, comitente: 1, createdAt: 1 },
        paging = {
            skip: page * 10,
            limit: 10
        };
...
exports.arrumandoLotesLances = function () {
    //console.log('to no update lote');
    
    //buscar todos os lances arrematados
    Lances_arrematado.find({},function(err, lances){
        console.log(err);
        console.log('qtd lances',lances.length);
        //buscar o lote
         for(var i=0; i < lances.length; i++){
            Lote.findOne({oldId:lances[i].LOTE_ID}, function(errLote, lote){
                
                if(lote){
                    console.log('id lote',lote._id);
                    var _condicional = false;
                    var _vendido = false;
                    var _encerrado = false;
                    var _movido = false;

                    if(lances[this.indexLance].VENDIDO == 'S'){
                        _vendido = true;
                    }
                    if(lances[this.indexLance].CONDICIONAL == 'S'){
                        _condicional = true;
                    }
                    if(lances[this.indexLance].ENCERRADO == 'S'){
                        _encerrado = true;
                    }

                        //Atualizar lote no leilao
                       Leilao.findOneAndUpdate(
                            { 'lotes.lote': lote._id},
                            {
                            "$set": {
                                "lotes.$.encerrado": _encerrado, "lotes.$.vendido": _vendido,
                                "lotes.$.lanceAtual.vencedor": _vendido, "lotes.$.condicional": _condicional
                            }
                            },function(err,retorno){
                                console.log('atualizou o leilao');
                             }
                       );


                       //Atualizar no lote
                       Lote.findByIdAndUpdate(lote._id,{vendido:_vendido, disponivel:false}, function(err,ret){
                           console.log('atualizou o lote');
                       });
                }
            }.bind({indexLance:i}));
         }
