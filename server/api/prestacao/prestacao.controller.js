...
// Wesley Inacio - 30/05/2017 - Contador
var Count = require("../count/count.model");
...
exports.searchPrestacao = function(req, res) {
    ...
    var user = req.user,
        ...
        query = { status: { $in: [3, 4] }, 'lotes.deleted': false },
        fields = {
            comitentes: 1, leilaoId: 1, description: 1, status: 1, active: 1, lotes: 1, usuariosHabilitados: 1, notaGerada: 1, oldId: 1
        },
        paging = {
            skip: page * 10,
            limit: 10
...
exports.gerarNota = function(req, res) {
    ...
    Leilao.findOne({ 'lotes._id': _loteId })
        .populate('lotes.lote', 'id description details comitentes')
        .populate('lotes.lanceAtual.comprador', 'simpleName addressStreet addressNumber addressState addressCity_ORIG addressState_ORIG addressCity postalCode phone doc1 doc2')
        .populate('lotes.propostaAceita.comprador', 'simpleName addressStreet addressNumber addressState addressCity_ORIG addressState_ORIG addressCity postalCode phone doc1 doc2')
        .deepPopulate('lotes.lanceAtual.comprador.addressState lotes.lanceAtual.comprador.addressCity lotes.propostaAceita.comprador.addressState lotes.propostaAceita.comprador.addressCity lotes.lote.comitente.razaoSocial', {
            populate: {
                'lotes.lanceAtual.comprador.addressState': { select: 'nome_estado' },
                'lotes.lanceAtual.comprador.addressCity': { select: 'nome_cidade' },
                'lotes.propostaAceita.comprador.addressState': { select: 'nome_estado' },
                'lotes.propostaAceita.comprador.addressCity': { select: 'nome_cidade' },
                'lotes.lote.comitente.razaoSocial': { select: 'razaoSocial' }
            }
        })
        .exec(
        function(err, leilao) {
            ...
            // Wesley Inacio - 24/05/2017
            var notaArrematacao = {
                empresa: "Leiloar Participações e Negócios Ltda",
                arrematante: "",
                endereco: "",
                numero: "",
                cidade: "",
                ...
                taxa: 0,
                total: 0,
                // find para obter a numeracao da nota atual.
                numeracao: 0
            };
            ...
            for(var i = 0; i < leilao.lotes.length; i++){
                ...
                if(leilao.lotes[i]._id == _loteId){

                    var propostaAceita = leilao.lotes[i].propostaAceita,
                        _lote  = leilao.lotes[i].lanceAtual;

                    // verificar se TEM prosposta
                    if( propostaAceita ){
                        _minValue = propostaAceita.valor;
                        _maxValue = propostaAceita.valor;
                        notaArrematacao.subTotal = propostaAceita.valor;
                    } else {
                        _minValue = _lote.valor;
                        _maxValue = _lote.valor;
                        notaArrematacao.subTotal = _lote.valor;
                    }
                    
                    Taxa.findOne({
                        minValue: { $lte: _minValue },
                        maxValue: { $gte: _maxValue },
                        active: true
                    }, function(err, taxa) {
                        ...
                        if( taxa ){
                            ...
                            if(leilao.lotes[i].lote.comitente.razaoSocial){
                                notaArrematacao.empresa = leilao.lotes[i].lote.comitente.razaoSocial;
                            }

                            // verificar se TEM prosposta
                            if( propostaAceita ){
                                notaArrematacao.comissao = ( (propostaAceita.valor * leilao.lotes[i].comissao) / 100 );
                                notaArrematacao.total = propostaAceita.valor + ( (propostaAceita.valor * leilao.lotes[i].comissao) / 100 ) + taxa.appTax;
                                notaArrematacao.lotes[0].valor = propostaAceita.valor;

                                // evitando a duplicação de código
                                _lote = propostaAceita;

                            } else {
                                notaArrematacao.comissao = ( (_lote.valor * leilao.lotes[i].comissao) / 100 );
                                notaArrematacao.total = _lote.valor + ( (_lote.valor * leilao.lotes[i].comissao) / 100 ) + taxa.appTax;
                                notaArrematacao.lotes[0].valor = _lote.valor;

                            }

                            // verificar se o arrematante é pessoa física ou juridica(razaoSocial)
                            if(_lote.comprador.name){
                                notaArrematacao.arrematante = _lote.comprador.name;
                                if(_lote.comprador.lastName){
                                    notaArrematacao.arrematante += " " + _lote.comprador.lastName;
                                }
                            }else{
                                 notaArrematacao.arrematante = "Arrematante Indefinido";
                            }

                            if(_lote.comprador.addressStreet){
                                notaArrematacao.endereco = _lote.comprador.addressStreet;
                            }else{
                                notaArrematacao.endereco = "Endereço Indefinido";
                            }

                            if(_lote.comprador.addressNumber){
                                notaArrematacao.numero = _lote.comprador.addressNumber;
                            }else{
                                notaArrematacao.numero = "Número Indefinido";
                            }

                            // addressCity_ORIG é dado da base antiga
                            if(_lote.comprador.addressCity_ORIG == undefined){
                                if(_lote.comprador.addressCity){
                                    notaArrematacao.cidade = _lote.comprador.addressCity.nome_cidade;    
                                }else{
                                    notaArrematacao.cidade = "Cidade Indefinido";
                                }
                            }else{
                                 notaArrematacao.cidade = _lote.comprador.addressCity_ORIG;
                            }

                            // addressState_ORIG é dado da base antiga
                            if(_lote.comprador.addressState_ORIG == undefined){
                                if(_lote.comprador.addressState){
                                    notaArrematacao.estado = _lote.comprador.addressState.nome_estado;
                                }else{
                                     notaArrematacao.estado = "Estado Indefinido";
                                }
                            }else{
                                notaArrematacao.estado = _lote.comprador.addressState_ORIG;
                            }

                            if(_lote.comprador.postalCode){
                                notaArrematacao.cep = _lote.comprador.postalCode;
                            }else{
                                notaArrematacao.cep = "CEP Indefinido";
                            }

                            if(_lote.comprador.phone){
                                notaArrematacao.telefone = _lote.comprador.phone;
                            }else{
                                notaArrematacao.telefone = "Telefone Indefinido";
                            }

                            if(_lote.comprador.doc1){
                                notaArrematacao.doc1 = _lote.comprador.doc1;
                            }else{
                                notaArrematacao.doc1 = "CPF Indefinido";
                            }

                            if(_lote.comprador.doc2){
                                notaArrematacao.doc2 = _lote.comprador.doc2;
                            }else{
                                notaArrematacao.doc2 = "RG Indefinido";
                            }

                            //se não encontrar, sera ZERO - DEFAULT
                            if(leilao.lotes[i].numero){
                                notaArrematacao.lotes[0].numero = leilao.lotes[i].numero;
                            }

                            if(leilao.lotes[i].lote.description){
                                notaArrematacao.lotes[0].descricao = leilao.lotes[i].lote.description;
                            }else{
                                 notaArrematacao.lotes[0].descricao = "Descrição Indefinido";
                            }
                            ...
                            notaArrematacao.autenticacao = mongoose.Types.ObjectId();

                            notaArrematacao.taxa = taxa.appTax;                         

                            console.log("notaArrematacao: ", notaArrematacao);

                            Count.find({name: "prestacao_nota_arrematacao"}, function(err, dataCount){
                                ...
                                for(var j = 0; j < dataCount.length; j++){
                                    if(dataCount[j].name == "prestacao_nota_arrematacao"){
                                        // set numeracao
                                        notaArrematacao.numeracao = dataCount[j].counter;
                                        nextCount.counter = dataCount[j].counter + 1;
                                        nextCount._id = dataCount[j]._id;
                                        break;
                                    }
                                }

                                console.log("notaArrematacao_new: ", notaArrematacao);

                                Leilao.update({ 'lotes._id': _loteId }, { $set: { 'lotes.$.nota': notaArrematacao, 'lotes.$.notaGerada': true } },
                                    function(err, result) {
                                        ...
                                        // se conseguir gerar a NA, então atualizar a numeracao do NA
                                        Count.update({"_id" : nextCount._id}, { $set: { counter: nextCount.counter } }, function(err){
                                            if(err){
                                                console.log(err);
                                                return handleError(res, err);
                                            }
...
exports.getNota = function(req, res) {
    Leilao.findOne({ 'lotes._id': req.params.id },
        function(err, leilao) {
            ...
            // Wesley Inacio - 25/05/2017
            var _lote = null;
            for(var i = 0; i < leilao.lotes.length; i++){
                if(leilao.lotes[i]._id == req.params.id){
                    console.log("encontrar lote i-d", req.params.id);
                    _lote = leilao.lotes[i].nota
                    break;
                }
            }
            console.log("aqui porra: ", _lote);
            // res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
            // res.write(leilao.lotes[0].boletoHtml);
            // res.end();
            console.log('retona jsson');
            return res.json(200, { nota: _lote });
        });
